 ...
 * @since Moodle 3.2
 */
function lti_check_updates_since($lti, $cm, $context, $from, $filter = array()) {
    global $DB, $USER;
    ...
    $updates = course_check_module_updates_since($lti, $cm, $context, $from, array(), $filter);

    // Check if there is a new submission.
    $select = 'ltiid = :id AND userid = :userid AND (datesubmitted > :since1 OR dateupdated > :since2)';
    $params = array('id' => $lti->id, 'userid' => $USER->id, 'since1' => $from, 'since2' => $from);
