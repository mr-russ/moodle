 ...
 * @since  Moodle 3.2
 */
function course_check_updates($course, $tocheck) {
    global $CFG, $DB;
        ...
        if ($instance['contextlevel'] == 'module') {
            ...
            if (!$cm->uservisible) {
                $warnings[] = array(
                    'item' => 'module',
                    'itemid' => $instance['id'],
                    'warningcode' => 'nonvisible',
                    'message' => 'Non visible module'
                );
                continue;
            }
            // Check module support for callback.
            $checkfunction = $cm->modname . '_check_updates_since';
            if (!isset($modulescallbacksupport[$cm->modname])) {
                $modpath = $CFG->dirroot . '/mod/' . $cm->modname . '/lib.php';
                require_once($modpath);
                $modulescallbacksupport[$cm->modname] = function_exists($checkfunction);
            }
            if (!$modulescallbacksupport[$cm->modname]) {
                $warnings[] = array(
                    'item' => 'module',
                    'itemid' => $instance['id'],
            ...
            $instances[] = array(
                ...
                'id' => $instance['id'],
                'updates' => call_user_func($checkfunction, $modinstance, $cm, $cm->context,
                                                $instance['since'], $instance['filter'])
            );
 ...
 * @since  Moodle 3.2
 */
function course_check_module_updates_since($mod, $cm, $context, $from, $fileareas = array(), $filter = array()) {
    global $DB, $CFG, $USER;
    ...
    if ($supportcompletion and (empty($filter) or in_array('configuration', $filter))) {
    ...
    if ($supportgrades and (empty($filter) or in_array('grades', $filter))) {
        require_once($CFG->libdir . '/gradelib.php');
